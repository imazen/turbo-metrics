//! sRGB→positive-XYB conversion matching zensim's CPU implementation.
//!
//! The CPU reference lives in `zen/zensim/zensim/src/color.rs` as
//! `srgb_to_positive_xyb_planar_inner`. Constants and math are mirrored
//! here. The kernel takes packed sRGB u8 RGB input (pitched 2D image)
//! and produces three planar f32 buffers (X, Y, B) at a padded width.
//!
//! Steps per pixel:
//!  1. sRGB u8 → linear f32 via LUT
//!  2. Opsin-like 3x3 matrix + bias, clipped to ≥0
//!  3. Halley-iterated cbrt (2 iterations) — sufficient for metric use
//!  4. XYB with absorbance_bias, then the "positive" shift:
//!       X' = X * 14 + 0.42
//!       Y' = Y + 0.01
//!       B' = (B - Y) + 0.55
...
// Matrix + bias constants, match zensim CPU.
const K_M02: f32 = 0.078;
const K_M00: f32 = 0.30;
const K_M01: f32 = 1.0 - K_M02 - K_M00; // 0.622
const K_M12: f32 = 0.078;
const K_M10: f32 = 0.23;
const K_M11: f32 = 1.0 - K_M12 - K_M10; // 0.692
const K_M20: f32 = 0.243_422_69;
const K_M21: f32 = 0.204_767_45;
const K_M22: f32 = 1.0 - K_M20 - K_M21; // ~0.551809...
const K_B0: f32 = 0.003_793_073_4;

// sRGB decode table: values match the CPU's `linear_srgb::default::srgb_u8_to_linear`.
// (Copied from ssimulacra2-cuda-kernel — both metrics use the same LUT.)
#[rustfmt::skip]
...
// Cube-root bit-manipulation initial estimate, matching CPU zensim's
// `cbrtf_initial`.
#[inline(always)]
unsafe fn cbrtf_initial(x: f32) -> f32 {
...
/// Two-iteration Halley cbrt; matches CPU zensim's `cbrtf_fast` exactly
/// (assuming identical FMA contraction, which ptxas generally matches
/// AVX2 for pure mul_add chains).
#[inline(always)]
unsafe fn cbrtf_fast(x: f32) -> f32 {
    if x == 0.0 {
        return 0.0;
    }
...
/// sRGB-packed-u8 → planar positive XYB.
///
/// `src` is a (width × height × 3)-byte image with row pitch `src_pitch`
/// bytes. `x_out`, `y_out`, `b_out` are planar f32 buffers with row
/// pitch `dst_pitch` **bytes** (so stride = dst_pitch/4 floats). Pitched
/// writes mirror how the ssim2/butter kernels expose padded strides for
/// coalesced access.
///
/// One thread per pixel; grid = ceil(width / 32) × ceil(height / 8).
#[unsafe(no_mangle)]
pub unsafe extern "ptx-kernel" fn srgb_to_positive_xyb_kernel(
    src: *const u8,
    src_pitch: usize,
    x_out: *mut f32,
    y_out: *mut f32,
    ...
    let row = src.byte_add(y * src_pitch);
    let r_u = *row.add(x * 3);
    let g_u = *row.add(x * 3 + 1);
    let b_u = *row.add(x * 3 + 2);

    let r = SRGB8_TO_LINEARF32_LUT[r_u as usize];
    let g = SRGB8_TO_LINEARF32_LUT[g_u as usize];
    let b = SRGB8_TO_LINEARF32_LUT[b_u as usize];
    ...
    // Positive shift.
    let x_pos = x_val.mul_add(14.0, 0.42);
    let y_pos = y_val + 0.01;
    let b_pos = b_val + 0.55;

    let x_row = x_out.byte_add(y * dst_pitch);
    let y_row = y_out.byte_add(y * dst_pitch);
    let b_row = b_out.byte_add(y * dst_pitch);
    *x_row.add(x) = x_pos;
    *y_row.add(x) = y_pos;
    *b_row.add(x) = b_pos;
}
