...
pub unsafe extern "ptx-kernel" fn srgb_to_linear_u8_lookup(
    ...
    *dst.byte_add(y * dst_pitch).add(x) = SRGB8_TO_LINEARF32_LUT[v as usize];
}

unsafe fn srgb_to_linear<const BITDEPTH: usize>(
    src: *const <Bitdepth<BITDEPTH> as Sample>::Type,
    src_pitch: usize,
    dst: *mut f32,
    dst_pitch: usize,
    width: usize,
...
pub unsafe extern "ptx-kernel" fn srgb_to_linear_f32(
    ...
    let v = src.byte_add(y * src_pitch).add(x).read();
    *dst.byte_add(y * dst_pitch).add(x) = srgb_inverse_oetf(v);
}
